...
export const exercises = pgTable(
    "exercises",
    {
        ...
        // Soft delete: an exercise with logged history can't be hard-deleted
        // (the RESTRICT FKs protect the logs), so it's archived instead —
        // hidden from every picker, history intact in Analyze, restorable.
        // NULL = active.
    ...
    (table) => [
        ...
        check(
            "exercises_target_weight_kg_check",
...
export const workoutExercises = pgTable(
    ...
    (table) => [
        unique("workout_exercises_workout_position_unique").on(
            table.workoutId,
            table.position,
        ),
        check(
            "workout_exercises_target_sets_check",
            sql`${table.targetSets} >= 1`,
        ),
    ],
);
...
export const loggedSets = pgTable(
    ...
    (table) => [
        ...
        index("logged_sets_user_exercise_performed_idx").on(
            table.userId,
            table.exerciseId,
            table.performedAt,
        ),
        // History / recent-activity views.
        index("logged_sets_user_performed_idx").on(
            table.userId,
            table.performedAt,
        ),
        ...
        check(
            "logged_sets_reps_check",
            sql`${table.reps} IS NULL OR ${table.reps} >= 0`,
        ),
        check(
            "logged_sets_weight_kg_check",
        ...
        check(
            "logged_sets_planned_reps_check",
            sql`${table.plannedReps} IS NULL OR ${table.plannedReps} >= 0`,
        ),
        check(
