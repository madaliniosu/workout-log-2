...
export function AddExerciseModal({
    ...
    function select(exercise: LogExerciseOption) {
        ...
        setPlannedDistanceMeters(
            exercise.targetDistanceMeters?.toString() ?? "",
        );
    }
    ...
    function submit(e: React.FormEvent<HTMLFormElement>) {
        ...
        onAdd({
            ...
            plannedWeightKg:
                plannedWeightKg === "" ? null : Number(plannedWeightKg),
            ...
            <form onSubmit={submit} className="flex flex-col gap-4">
                {selected === null ? (
                    <SearchList
                        ...
                        getName={(exercise) => exercise.name}
                        onSelect={select}
                    />
                ) : (
                    <>
                        ...
                        {selected.tracksWeight && (
                            <label className="flex flex-col gap-1">
                                ...
                                <input
                                    ...
                                    onChange={(e) =>
                                        setPlannedWeightKg(e.target.value)
                                    }
                                    className="h-11 w-full rounded-xl border border-border px-3 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-accent"
                                />
                            </label>
                        ...
                        {selected.tracksDistance && (
                            <label className="flex flex-col gap-1">
                                ...
                                <input
                                    ...
                                    onChange={(e) =>
                                        setPlannedDistanceMeters(e.target.value)
                                    }
                                    className="h-11 w-full rounded-xl border border-border px-3 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-accent"
                                />
                            </label>
